    ...
    until rules.game_over?
      if rules.player_turn.eql?(rules.player_one_marker)
        ...
        if @ai_player_one != nil
          spot = @ai_player_one.best_move(rules.board.clone, rules.player_turn)
        else
          spot = @input_output.ask_player_for_location_to_mark
          while rules.board.location_valid_to_mark?(location: spot, player_one_marker: rules.player_one_marker, player_two_marker: rules.player_two_marker) == false
            spot = @input_output.ask_player_for_location_to_mark
          end
        end
      else
        ...
        if @ai_player_two != nil
          spot = @ai_player_two.best_move(rules.board.clone, rules.player_turn)
        else
          spot = @input_output.ask_player_for_location_to_mark
          while rules.board.location_valid_to_mark?(location: spot, player_one_marker: rules.player_one_marker, player_two_marker: rules.player_two_marker) == false
            spot = @input_output.ask_player_for_location_to_mark
